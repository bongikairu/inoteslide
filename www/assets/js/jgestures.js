...
//									_$element.get(0).removeEventListener('mouseup', _onTouchend, false);
	...
	*/
	function _onTouchstart(event_) {
		...
		if(_iTouches<3) _$element.triggerHandler($.jGestures.events.touchstart,event_);
...
//			event_.currentTarget.addEventListener('mouseup', _onTouchend, false);
	...
	*/
	function _onTouchend(event_) {
		...
		// trigger custom notification
		if(_iTouches<3) __$element.triggerHandler($.jGestures.events.touchendStart,event_);
...
//			_oObj.hasTouchmoved = false;
	...
	*/
	function _onGestureend(event_) {
		...
		//var _iTouches = event_.changedTouches.length;

		//alert(event_.changedTouches);

		//alert(JSON.stringify(event_));

		//if(_iTouches!=2) alert('pinch with '+_iTouches+' fingers');
